...
- **Posts architecture (refactored Jul 22 2026):** each post lives in `app/writing/posts/<slug>.tsx` exporting `post: Post`; `app/writing/posts/index.ts` imports them all in display order (newest first) and exports the `posts` array consumed by `app/writing/[slug]/page.tsx`. **To publish a post:** (1) create `app/writing/posts/<slug>.tsx` (copy an existing file's shape), (2) add its import at the TOP of the import list in `index.ts` and prepend it in the array, (3) add the metadata entry to `app/lib/posts-data.ts` as before. voice-lint now scans only the changed post files.
